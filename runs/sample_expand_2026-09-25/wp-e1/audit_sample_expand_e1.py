#!/usr/bin/env python3
"""WP-E1: learning curve + ticker-day group permutation on freeze 001856a6.
...
def main() -> int:
    ...
    log(
        f"Permutation test: n={N_PERMS} group-level on train pool, score=val AUPRC",
        lines,
    )
    obs = fit_score(x_tr_full, y_tr_full, x_va, y_va, seed=0)
    observed_auprc = obs["val_auprc"]
    log(f"  observed val_auprc={observed_auprc:.6f}", lines)
    rng = np.random.default_rng(0)
    null_scores: list[float] = []
    for i in range(N_PERMS):
        y_perm = permute_labels_by_group(y_tr_full, g_tr, rng)
        try:
            m = fit_score(x_tr_full, y_perm, x_va, y_va, seed=0)
            null_scores.append(float(m["val_auprc"]))
        except ValueError as exc:
            log(f"  perm {i}: skip ({exc}); use prevalence as null score", lines)
        ...
        if (i + 1) % 25 == 0:
            ...
            # checkpoint null scores so a crash does not lose the curve work
            (OUT / "permutation_checkpoint.json").write_text(
                json.dumps({"n_done": i + 1, "observed_val_auprc": observed_auprc, "null_scores": null_scores}, indent=2)
            )
