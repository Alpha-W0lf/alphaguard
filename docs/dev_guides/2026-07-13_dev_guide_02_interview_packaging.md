...
- [ ] `INTERVIEW.md` at repo root with **≥15** gotcha Q&A covering themes listed in Phase A  
- [ ] `GETTING_STARTED.md` at repo root with clean-clone path (see Phase B)  
- [ ] README gains architecture mermaid (simplify ARCHITECTURE §4) + stack table; still vertical slice / not v1 complete; fixture ≠ Option B  
- [ ] `docs/assets/` has **≥2** checked-in screenshots: (1) terminal smoke, (2) curated envelope JSON — LS/Phoenix stub captions; home paths redacted; Agent 1 variance caption  
- [ ] Operator docs keep default `gemma4:e2b` honesty (412 / fallback)  
- [ ] No new pipeline features; no Kafka E2E; no Option B train; no real LS/Phoenix SDK; LICENSE not required in core DoD  
- [ ] Do not silently tick VISION MV boxes without Align-docs evidence pass  
- [ ] FAQ / captions reconcile VISION “LangSmith screenshots” wording via local-envelope fulfillment until H2 reversed  
...
- [ ] **A1.** Create `alphaguard/INTERVIEW.md` at **repo root** (not under `docs/`).  
- [ ] **A2.** Write **≥15** Q&A covering **all** required themes below. Exact question wording may be drafted in Implement; titles below are the checklist. Each theme must appear at least once (one Q may cover a tightly related pair if both ideas are explicit in the answer).
...
- [ ] **A3.** Optional 16th–25th Qs only if thin: e.g. FinBERT-not-in-smoke, confidence-is-trace-only, no neural reranker — do not bloat.  
- [ ] **A4.** Cross-check: no answer contradicts ARCHITECTURE §7 / AG1–AG3; no claim that packaging = v1 Done; no fake “LangSmith proven” language.  
- [ ] **A5.** Link from INTERVIEW.md to `docs/ARCHITECTURE.md` and `docs/VISION.md` (paths only; keep FAQ self-contained enough for drills).
...
- [ ] **B1.** Create `alphaguard/GETTING_STARTED.md` at **repo root**.  
- [ ] **B2.** Document clean-clone path in order:  
...
- [ ] **B3.** State defaults: `ALPHAGUARD_RAG_MODE=fixture`; Qdrant/Kafka optional later; smoke does **not** require Compose.  
- [ ] **B4.** One-line note: Compose file present ≠ Kafka maturity (point to INTERVIEW theme).  
- [ ] **B5.** One-line note: primary model missing → preflight may use fallback (soft warning-debt OK to mention briefly).  
- [ ] **B6.** Link back to README + ARCHITECTURE; do not tick VISION boxes from this file.

### Phase C — README diagram + stack table

- [ ] **C1.** Add a **simplified** mermaid flowchart derived from ARCHITECTURE §4. Prefer critical path: Replay fixtures → Replay runner → PipelineService → (fixture RetrievalHits / optional Qdrant) → Agent 1 → Agent 2 → Local run summary (+ LS/Phoenix best-effort stubs).  
- [ ] **C2.** Diagram honesty: Kafka may appear as optional / later path — do **not** draw Kafka as always-on for smoke. Preserve ARCHITECTURE wording intent: Kafka mandatory in architecture/Compose; **optional for smoke**.  
- [ ] **C3.** Add a **stack table** (or clear short table + link to VISION/ARCHITECTURE locked stack): Python/`uv`, LangGraph, Ollama (`gemma4:e2b`), XGBoost gate, fixture RAG default, Compose Kafka+Qdrant (optional for smoke), local envelope mandatory.  
- [ ] **C4.** Keep / strengthen: **vertical slice / not v1 complete**; `bundle_kind=fixture` ≠ Option B; LS/Phoenix stubs; Limitations still honest.  
- [ ] **C5.** Link `GETTING_STARTED.md` and `INTERVIEW.md` from README Docs section; update Limitations to remove “absent” once artifacts exist (only after files land).  
- [ ] **C6.** Trivial AGENTS.md one-liner OK if needed (“guide 02 packaging landed”) — do not reopen stack locks.
...
- [ ] **D0a.** With Kafka **down**, run from repo root: `make bundle` (if needed) then `make smoke` (default `gemma4:e2b` or documented fallback). Confirm exit 0.  
- [ ] **D0b.** Note the printed envelope path under `artifacts/runs/`.  
- [ ] **D0c.** Capture **terminal** screenshot of the successful smoke excerpt (proposal + decision + envelope path hint).  
- [ ] **D0d.** Open the envelope JSON; prepare a **curated** view for screenshot (pretty-print selected keys — see D2). Do not git-add `artifacts/`.

#### D1 — Agent redaction checklist (mandatory before commit)

- [ ] **D1a.** Create `docs/assets/` if missing.  
- [ ] **D1b.** **Redact** any absolute home paths (`/Users/...`, `/home/...`) from visible JSON / terminal crop — especially `obs.local_summary_path`. Prefer relative path text or `artifacts/runs/<id>.json` in captions.  
- [ ] **D1c.** Never paste secrets, `.env` contents, or API keys into images or captions.  
- [ ] **D1d.** Never edit JSON values to fake `obs.langsmith=ok` / Phoenix success for marketing. Capture honest stub statuses (`skipped` / `failed` / config-driven).  
- [ ] **D1e.** Do not invent UI screenshots of LangSmith or Phoenix.

#### D2 — Required assets (≥2)

- [ ] **D2a.** `docs/assets/smoke_terminal.png` (name flexible) — terminal smoke excerpt showing success + envelope path hint.  
- [ ] **D2b.** `docs/assets/run_envelope_curated.png` (name flexible) — curated envelope JSON showing at least: `status=success`, `rag_mode=fixture`, `resource_mode=replay_fixture`, `obs.langsmith` and `obs.phoenix` stub fields visible.  
- [ ] **D2c.** Captions (in README and/or `docs/assets/README.md` short note):  
...
- [ ] **D3.** Reference both images from README (and optionally GETTING_STARTED).  
- [ ] **D4.** Confirm `artifacts/` remains gitignored; only `docs/assets/*` images are committed.

### Phase E — Honesty pass + cross-links

- [ ] **E1.** Grep operator docs for accidental “qwen-only DoD,” “LangSmith proven,” “Option B complete,” or “v1 Done” claims; fix if introduced.  
- [ ] **E2.** Confirm gemma default + 412/fallback appears in GETTING_STARTED and remains consistent with README / AGENTS / `.env.example`.  
- [ ] **E3.** Confirm FAQ + captions state: local envelope fulfills packaging until H2 reversed; packaging-first soft override of ARCHITECTURE §15 cited once (INTERVIEW or GETTING_STARTED “why packaging now” note is enough — do not rewrite ARCHITECTURE in this guide).  
- [ ] **E4.** Confirm LICENSE was **not** added as a hard DoD item (optional follow-on only if human expands scope).  
- [ ] **E5.** Confirm no Compose `docker compose up` operator appendix was added as a guide-02 requirement.  
- [ ] **E6.** Stop. Do not start Kafka E2E, Option B, eval≥20, or Align-docs MV checkbox edits in this guide.
...
**Checked:** completeness vs context AC; step order A→E; screenshot capture/redaction DoD; blast radius; edge cases; soft pins locked; no Implement creep.

**Material edits this refine:** Status/readiness only — Write draft already pinned paths, theme table (15), human smoke + redaction checklist, LICENSE/Compose out of core DoD, §15 packaging-first soft override. No missing steps or DoD holes found.

**Honest call:** **Ready check next** (not more Refine). Still **not** authorized to Implement until Ready check + human.
...
- **Refine-dev-guide:** complete this pass — guide good to go as planning artifact.  
